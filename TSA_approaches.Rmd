...
```{r base_get_data}
data_df <- read.csv('https://www.gstatic.com/covid19/mobility/Global_Mobility_Report.csv',
                    stringsAsFactors = T)
...
colnames(filtered_data_df)[9:14] <- gsub("_percent_change_from_baseline",
                                      "",
                                      colnames(filtered_data_df)[9:14])
...
```{r xts}
data_xts <- xts(x = filtered_data_df$parks,order.by = filtered_data_df$date)
plot(data_xts)
```

```{r}
plot(data_xts['2020-06-01/'])
```
